...
function _draw()
	...
	if (Config.debug) then
		DebugVisualization.draw_camera_info(camera, ship_heading_dir, target_heading_dir, raycast_x, raycast_z)
	end

	-- Lighting debug (only visible when Config.debug_lighting is true)
	if Config.debug_lighting then
		DebugVisualization.draw_lighting_debug(light_yaw, light_pitch, get_light_direction, draw_line_3d, camera)
	end
